...
class SyntheticPhotoProcessor:
  ...
  def describe(
    ...
    """Print, return, or export a structured summary of all parameters.

    Args:
        return_str: If True, returns the formatted string instead of printing.
        indent: Indentation level for printed formatting.
        path: Optional file path ('.json' or '.txt') to save configuration.

    Returns:
        str | None: Human-readable summary if return_str=True.
    """
    import json
    ...
    pad = " " * indent
    lines = []
    lines.append("SyntheticPhotoProcessor configuration:")
    lines.append(f"{pad}--- Lighting & Texture ---")
    lines.append(f"{pad}top_bright        = {self.top_bright:.3f}")
    lines.append(f"{pad}bottom_dark       = {self.bottom_dark:.3f}")
    lines.append(f"{pad}texture_strength  = {self.texture_strength:.3f}")
    lines.append(f"{pad}texture_scale     = {self.texture_scale:.2f}")
    lines.append("")
    lines.append(f"{pad}--- Noise ---")
    lines.append(f"{pad}gaussian_std      = {self.gaussian_std:.3f}")
    lines.append(f"{pad}poisson_noise     = {self.poisson_noise}")
    lines.append(f"{pad}sp_amount         = {self.sp_amount:.3f}")
    lines.append(f"{pad}speckle_var       = {self.speckle_var:.3f}")
    lines.append(f"{pad}blur_sigma        = {self.blur_sigma:.3f}")
    lines.append("")
    lines.append(f"{pad}--- Optics ---")
    lines.append(f"{pad}tilt_x            = {self.tilt_x:.3f}")
    lines.append(f"{pad}tilt_y            = {self.tilt_y:.3f}")
    lines.append(f"{pad}focal_scale       = {self.focal_scale:.3f}")
    lines.append(f"{pad}k1                = {self.k1:.3f}")
    lines.append(f"{pad}k2                = {self.k2:.3f}")
    lines.append(f"{pad}pad_px            = {self.pad_px}")
    lines.append("")
    lines.append(f"{pad}--- Post-Optical ---")
    lines.append(f"{pad}vignette_strength = {self.vignette_strength:.3f}")
    lines.append(f"{pad}warm_strength     = {self.warm_strength:.3f}")

    # Join text for console or file
    text_summary = "\n".join(lines)
...
if __name__ == "__main__":
  ...
  processor.set_preset("smartphone") 

  # Log configuration
  processor.describe()
  
  # or save to metadata file
  config_str = processor.describe(return_str=True)
  with open("synthetic_photo_params.txt", "w") as f:
    f.write(config_str)  
  processor.describe(path="smartphone_config.json")

  result = processor.process(rgba)
  #cv2.imshow("Synthetic Lab Photo (Reusable Tool)", result)
  cv2.imshow("Synthetic Photo (Smartphone Preset)", result)
  cv2.waitKey(0)
