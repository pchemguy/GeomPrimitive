...
class SyntheticPhotoProcessor:
  ...
  def process_stages(self, rgba: np.ndarray) -> dict[str, np.ndarray]:
    ...
    stages["Lighting"] = self.apply_paper_lighting(stages["Base"])
    stages["Texture"]  = self.add_paper_texture(stages["Lighting"])
    stages["Noise"]    = self.apply_sensor_noise(stages["Texture"])
    stages["Optical"]  = self.apply_camera_effects(stages["Noise"])
    stages["Final"]    = self.add_vignette_and_color_shift(stages["Optical"])

    for name, img in stages.items():
      overlay = img.copy()
      cv2.putText(overlay, name.upper(),
                  (20, 40), cv2.FONT_HERSHEY_SIMPLEX,
                  1, (255, 0, 0), 2, cv2.LINE_AA)
      stages[name] = cv2.addWeighted(overlay, 0.5, img, 0.5, 0)
  ...
  # ------------------------------------------------------------------
  def save(self, path: str, rgba: np.ndarray) -> str:
    """Run full processing pipeline and save final image to disk.

    The output format is determined automatically by file extension.

    Args:
        path: Destination file path (e.g. "output.jpg" or "output.png").
        rgba: Input RGBA NumPy array from render_scene().

    Returns:
        The absolute path of the saved image.
    """
    img = self.process(rgba)

    # Determine format by extension
    ext = os.path.splitext(path)[1].lower()
    if ext not in [".png", ".jpg", ".jpeg", ".tif", ".tiff", ".bmp", ".webp"]:
      self.logger.warning(
        f"\u26A0\uFE0F  Unknown image extension '{ext}', "
        f"defaulting to PNG format.")
    ...
    if not ok:
      self.logger.warning(f"\u26A0\uFE0F  Failed to save image: {path}")
      return path

    self.logger.info(f"\u2705  Image saved -> {os.path.abspath(path)}")
