...
class SyntheticPhotoProcessor:
  ...
  # ------------------------------------------------------------------
  def describe(self, return_str: bool = False, indent: int = 2) -> str | None:
    """Print or return a formatted summary of all active parameters.

    Useful for dataset metadata, debugging, or reproducibility logs.

    Args:
        return_str: If True, returns the formatted string instead of printing.
        indent: Number of spaces for indentation in printed output.

    Returns:
        Formatted parameter summary (if return_str=True).
    """
    lines = []
    pad = " " * indent
