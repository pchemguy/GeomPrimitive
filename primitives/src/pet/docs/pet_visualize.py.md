Here is a clean, minimal, **zero-boilerplate `pet_visualize.py` module** providing **instant matplotlib debugging** of any pipeline stage.

This module is separate from the heavy “debug report” we built before.  
**Goal:** QUICK LOOK, zero friction, while working interactively.

Everything here is:
- Lightweight
- Pure matplotlib
- Convenient during development
- Compatible with your PETConfig + pipeline
- Zero OpenCV writing unless explicitly requested

You’ll get:

### ✔ show_images()

Quick grid of BGR images (converted to RGB) with titles.

### ✔ show_mask_overlay()

Display any mask overlayed on BGR image using alpha compositing.

### ✔ show_hist()

Interactive histogram diagnostic.

### ✔ show_lab_planes()

Visualize L*, a*, b* channels quickly.

### ✔ show_pipeline()

One-call combined visualization of pipeline stages using your `stages` dict.
...
```python
from pet_visualize import show_mask_overlay
...
```python
from pet_visualize import show_hist
show_hist(final_img, "Final Image Histogram")
```

LAB visualization:

```python
from pet_visualize import show_lab_planes
show_lab_planes(img)
```
...
### Perfect during development and tuning:

- No file output (unless you save from matplotlib)
- Immediate feedback
- Very readable grids
- Reusable for FFT/perspective tests
- Works with PET pipeline stages dictionary structure

### Complements `pet_debug.py`

- `pet_debug` = structured logging + file writes + composite images
- `pet_visualize` = immediate visual feedback for humans

Together they give a truly complete development toolkit.
