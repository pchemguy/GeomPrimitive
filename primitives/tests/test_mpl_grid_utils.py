...
def test_generate_grid_collections_orthogonal_no_jitter() -> None:
    ...
    assert len(x_major.get_segments()) == 5
    assert len(y_major.get_segments()) == 5

    # For step = 2.5, indices are k in [-5, 5] -> 9 lines.
    assert len(x_minor.get_segments()) == 9
    assert len(y_minor.get_segments()) == 9
