...
_worker = None  # Per-process worker instance (created by worker_init)


# -----------------------------------------------------------------------------
# Colorized logging formatter (console only)
# -----------------------------------------------------------------------------
class ColorFormatter(logging.Formatter):
  ...
  def format(self, record: logging.LogRecord) -> str:
    ...
    record.process_str = f"{record.process:5d}"        # PID padded to width 5
    record.level_str = f"{record.levelname:<5s}"        # LEVEL padded left to 5
    return (
...
def configure_logging(
  ...
  """Configure colorized console + rotating file logging. Return this run's log path."""
  colorama_init()
  ...
  # Monochrome format for file handler, aligned PID/LEVEL
...
class RunSummary:
  ...
  def finalize(self):
    ...
    # File: monochrome footer (also goes through logger)
    mono_lines = [
        "",
        "=" * 78,
        "RUN SUMMARY",
...
# -----------------------------------------------------------------------------
# Worker class (no flow here - only primitives)
# -----------------------------------------------------------------------------
class SyntheticImageWorker:
  """
  Per-process holder for Matplotlib state and primitive drawing ops.
  No flow orchestration here by design.
  """
  ...
  def __enter__(self):  # optional, if you want to use context manager locally
    return self
  ...
  def _create_canvas(self):
    # Only create Figure/Axes; no axis-limits/appearance here.
    width_in = self.img_size[0] / 100
    height_in = self.img_size[1] / 100
    fig, ax = plt.subplots(figsize=(width_in, height_in), frameon=False)
    return fig, ax

  def plot_reset(self):
    """Reset the Axes for a new frame (no flow logic)."""
    self.ax.cla()
  ...
  def save_image(self, output_path: PathLike):
...
def main_worker(output_path: PathLike) -> Tuple[Optional[Path], Optional[Exception]]:
  """
  Flow/orchestration lives here (per job, per process):
    - reset
  ...
  try:
    _worker.plot_reset()
    with plt.xkcd():  # stylistic context is flow-level business logic
      _worker.draw_spline()
      _worker.draw_ellipse()
      _worker.draw_line()
    out = _worker.save_image(output_path)
    return out, None
